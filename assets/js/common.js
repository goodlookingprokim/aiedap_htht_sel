...
window.addEventListener('load', function() {
    // 페이지 로드 후 하단 요소 확인 및 제거
    function removeUnwantedBottomElements() {
        const bodyChildren = Array.from(document.body.children);
        const lastElements = bodyChildren.slice(-3); // 마지막 3개 요소 확인
        
        lastElements.forEach(element => {
            // script, style, 정상적인 요소가 아닌 경우 제거
            if (element && 
                ...
                !element.id &&
                (element.tagName === 'DIV' || element.tagName === 'BUTTON' || element.tagName === 'A')) {
                console.log('Removing unwanted bottom element:', element);
                element.remove();
            }
        });
    }
        ...
        mutations.forEach((mutation) => {
            mutation.addedNodes.forEach((node) => {
                if (node.nodeType === 1 && 
                    ...
                    // 잠시 후 확인 (요소가 완전히 렌더링된 후)
                    setTimeout(() => {
                        if (!node.classList.length && 
                            !node.id &&
                            node.tagName !== 'SCRIPT' &&
                            node.tagName !== 'STYLE' &&
                            (node.tagName === 'DIV' || node.tagName === 'BUTTON' || node.tagName === 'A')) {
                            console.log('Removing dynamically added bottom element:', node);
                            node.remove();
                        }
                    }, 100);
                }
